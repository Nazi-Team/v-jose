...
let handler = m => m
handler.before = async function (m, { sock, m.isAdmin, isBotAdmin, isOwner }) { 
...
if (isToxic && chat.antitoxic && !isOwner && !m.isAdmin && isBotAdmin) {
if (chat.delete) return sock.reply(m.chat, mid.mAdvertencia + mid.mAntiDelete, m)
user.warn += 1
if (!(user.warn >= 4)) {
await sock.reply(m.chat, mid.antitoxic1(isToxic, m, user), m)
...
user.warn = 0
await sock.reply(m.chat, mid.antitoxic2(isToxic, m, user), m)
await sock.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
